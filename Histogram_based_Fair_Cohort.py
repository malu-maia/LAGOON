...
class Histogram_based_Fair_Cohort:
    ...
    def fit(self, X: np.array, y: np.array): # training data as input
        if 'adult' in self.dataset_name:
            df_to_group = pd.DataFrame(X, columns=self.dataset_for_fairness.columns[1:-1])
        else:
            df_to_group = pd.DataFrame(X, columns=self.dataset_for_fairness.columns[:-1])
        groups = list(df_to_group.groupby(list(df_to_group.columns)).groups.values())
        groups = [np.ravel(groups[i]).tolist() for i in range(len(groups))]
        for i in range(len(groups)):
            for elem in groups[i]:
                self.histograms[elem] = sum(y[groups[i]])
        self.histograms = dict(sorted(self.histograms.items()))
        return self.histograms
    
    def predict(self, X_train: np.ndarray, X_test: np.ndarray): # test data as input
        y_pred = []
        for instance in X_test:
            id_most_similar = self.find_most_similar_instance_in_training(X_train, instance)
            y_pred.append(self.histograms[id_most_similar])
        return y_pred
    
    def find_most_similar_instance_in_training(self, X_train: np.ndarray, instance: np.ndarray) -> int: 
        # receives an instance from test as input and finds the most similar instance in training data
        id_most_similar = None
        dist = np.inf
        for j in range(len(X_train)):
            dist_from_j = self.get_dist(X_train[j], instance)
            if dist_from_j < dist:
                id_most_similar = j
                dist = dist_from_j
    ...
    def store_information(self, lamb: float, X: np.ndarray, y_test: np.ndarray, y_pred: np.ndarray, run=0):
        ...
        with open('outputs/HBFC_lamb{}_run{}_{}_results.txt'.format(lamb, run, self.dataset_name), 'w') as f:
            f.write('\n'.join(data))
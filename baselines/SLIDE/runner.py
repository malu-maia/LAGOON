...
def runner(args) :
    ...
    # device
    device = torch.device("cuda:{}".format(args.device)) if torch.cuda.is_available() else torch.device("cpu") #torch.cuda('cpu')

    
    # load data
    if args.dataset == 'adult':
        (xs_train, x_train, y_train, s_train), (xs_test, x_test, y_test, s_test) = load_adult_dataset(sample=args.sample)
    elif args.dataset == 'german':
        (xs_train, x_train, y_train, s_train), (xs_test, x_test, y_test, s_test) = load_german_dataset(sample=args.sample)
    elif args.dataset == 'compas':
        (xs_train, x_train, y_train, s_train), (xs_test, x_test, y_test, s_test) = load_compas_dataset(sample=args.sample)
    else:
    ...
    for epoch in tqdm(range(args.epochs), desc = "DI learning") :
        ...
        #learning_stats = {"auc" : [], "bacc" : [], "di" : []}
        ###### uif
        #learning_stats = {"auc" : [], "bacc" : [], "uif" : []}

        ##### uif with consistency
        learning_stats = {"auc" : [], "uif" : [], "consistency": []}
        ...
        if (epoch + 1) % 500 == 0 or epoch == args.epochs - 1:
            ...
            with torch.no_grad() :
                ...
                ##### UIF
                inputs = list(testloader_0_)[0][0].float().to(device)
                test_perfs = evaluate_uif(inputs, preds_, all_targets, sensitives)
